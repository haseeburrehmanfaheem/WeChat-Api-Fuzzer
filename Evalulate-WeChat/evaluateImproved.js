...
global.evaluate = async function (finalScript) {
  if (!evaluated) {
...
async function runEach() {
  const inputD = await fs.readFile("./undocumentedAPIs1.txt");
        ...
        try{
            wx.${element}(
                {success: function (res) 
                    {globalThis.resultSet["${element}"]=res;},
                fail: function(err)
                    {globalThis.resultSet["${element}"]=err;}});
...
/*
(()=>{return globalThis.resultSet;})()

(()=>{try{wx.getLocation({success: function (res) {globalThis.resultSet["getLocation"]=res;},fail: function(err){globalThis.resultSet["getLocation"]=err;}});} catch(err){globalThis.resultSet["getLocation"]=err;}})()
