...
var nameCheck = ["_zn2v87isolate12createparams"/*"_ZN2v82V810InitializeEi", "_ZN2v87Isolate10GetCurrentEv" ,"_ZN2v86Script3RunENS_5LocalINS_7ContextEEE", "_ZN2v86String11NewFromUtf8EPNS_7IsolateEPKcNS_13NewStringTypeEi"*/]
...
var nameCheck3 = ["_ZN2v87Isolate","_ZN2v86Script"]
...
function hookCreateParams(nameCheck){
    ...
    var exports  = Module.enumerateExports("libzeusv8.so");

    exports.forEach(exp=>{
        if(exp.type === "function" && nameCheck.some(name => exp.name.toLowerCase().includes(name.toLowerCase()))){
            if( exp.name.toLowerCase().includes("enter") ||   exp.name.toLowerCase().includes("exit")    ||  exp.name.toLowerCase().includes("get") || exp.name.toLowerCase().includes("dispose") ||  exp.name.toLowerCase().includes("script") || exp.name.toLowerCase().includes("new")){
            ...
            Interceptor.attach(exp.address, {
                ...
                onEnter: function(args){
                    ...
                    if((exp.name.includes("Isolate")) && (exp.name.includes("Exit") || exp.name.includes("Enter")) || exp.name.includes("Context")){
                        console.log(`Called ${exp.name}`);
                        console.log("isolate = " , params )
                    }
